package cn.slimsmart.zookeeper.demo.curator.extensions.discovery;
...
public class DiscoveryTest {
	...
	private static final String PATH = "/curator/discovery";
	...
	private static void processCommands(ServiceDiscovery<InstanceDetails> serviceDiscovery, Map<String, ServiceProvider<InstanceDetails>> providers,
		...
		printHelp();
		List<ServerTest> servers = Lists.newArrayList();
		try {
	...
	private static void deleteInstance(String[] args, String command, List<ServerTest> servers) {
		...
		ServerTest server = Iterables.find(servers, new Predicate<ServerTest>() {
			@Override
			public boolean apply(ServerTest server) {
	...
	private static void addInstance(String[] args, CuratorFramework client, String command, List<ServerTest> servers) throws Exception {
		...
		String serviceName = args[0];
		ServerTest server = new ServerTest(client, PATH, serviceName, description.toString());
		servers.add(server);
		server.start();
